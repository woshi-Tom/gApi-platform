...
> 来源: 用户测试 — 模型管理编辑保存报错
> 状态: ✅ 已修复 (v1.3.0)
> 修复提交: c329bff, bdf8bc9
> 采用方案: 方案 B（后端兼容层 `parseAbilityTypes()`）+ 前端 placeholder 修正
...
|--------|------|
| 是否修复 | ✅ 已修复 |
| 采用方案 | 方案 B：后端 `parseAbilityTypes()` 兼容字符串和数组两种格式，前端 placeholder 修正 |
| 责任人 | TOM |
| 排期 | v1.3.0（2026-05-13） |
