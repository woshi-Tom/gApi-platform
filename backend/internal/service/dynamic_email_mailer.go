...
func NewDynamicEmailMailer(settingsSvc *SettingsService) *DynamicEmailMailer {
	return &DynamicEmailMailer{settingsSvc: settingsSvc}
}

func (m *DynamicEmailMailer) SendVerificationEmail(toEmail, code, purpose string) error {
	cfg, err := m.settingsSvc.GetSMTPConfig()
	if err != nil {
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
    <div style="background: linear-gradient(135deg, #e6a23c 0%%, #d4940a 100%%); padding: 30px; text-align: center; border-radius: 12px 12px 0 0;">
    ...
    <div style="background: #fff; padding: 30px; border: 1px solid #e4e7ed; border-top: none; border-radius: 0 0 12px 12px;">
        <h2 style="color: #303133; margin-top: 0;">密码重置验证码</h2>
        ...
        <div style="background: #f5f7fa; padding: 20px; text-align: center; border-radius: 8px; margin: 20px 0;">
            <span style="font-size: 32px; font-weight: bold; color: #e6a23c; letter-spacing: 8px;">%s</span>
        </div>
...
`, code)
	...
	return m.sendEmail(cfg, toEmail, subject, content)
}

func (m *DynamicEmailMailer) SendPasswordResetEmail(toEmail, resetLink string) error {
	cfg, err := m.settingsSvc.GetSMTPConfig()
	if err != nil {
		return fmt.Errorf("failed to get SMTP config: %w", err)
	}
...
func (m *DynamicEmailMailer) sendEmail(cfg *SMTPConfig, to, subject, htmlContent string) error {
	from := cfg.FromEmail
	if cfg.FromName != "" {
	...
	if cfg.UseTLS {
		return m.sendWithTLS(cfg, addr, auth, cfg.FromEmail, []string{to}, message.String())
	}

	return smtp.SendMail(addr, auth, cfg.Username, []string{to}, []byte(message.String()))
}

func (m *DynamicEmailMailer) sendWithTLS(cfg *SMTPConfig, addr string, auth smtp.Auth, from string, to []string, msg string) error {
